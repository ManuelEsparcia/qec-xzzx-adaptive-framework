...
def _build_decoder(distance: int = 3, rounds: int = 3, p: float = 0.01) -> MWPMDecoderWithSoftInfo:
    """
    Helper for construir un decoder listo for tests.
    """
...
def test_decode_with_confidence_output_structure() -> None:
    """
    decode_with_confidence must devolver:
    - prediction
    - soft_info with required keys
    - decode_time
    """
    decoder = _build_decoder()
    ...
    assert prediction.size >= 1  # at least one logical observable is expected
...
def test_invalid_syndrome_shape_raises() -> None:
    """
    Síndrome with shape incorrecto must fail.
    """
    decoder = _build_decoder()
...
def test_invalid_circuit_type_raises() -> None:
    """
    Constructor must validar tipo de circuit.
    """
...
def test_custom_confidence_config_is_accepted() -> None:
    """
    Must aceptar una config custom de confianza.
    """
    circuit = generate_xzzx_circuit(
        distance=3,
        rounds=2,
...
def test_benchmark_output_contract() -> None:
    """
    benchmark must return the main keys and expected types.
    """
    decoder = _build_decoder(distance=3, rounds=2, p=0.02)
    ...
    # error_rate suele estar en [0,1] (nan solo en edge-cases without observables)
